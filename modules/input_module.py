...
def render_sidebar() -> InputData:
    ...
    st.markdown('<h4 style="margin-top:0;margin-bottom:0.5rem">📋 输入参数</h4>', unsafe_allow_html=True)
    ...
    with st.expander("评估对象", expanded=True):
        mode = st.selectbox(
            ...
            key=_wkey("input_mode"),
        )
        your_age = st.number_input(
            "你的年龄", min_value=0, max_value=100,
            value=defaults.your_age, key=_wkey("input_your_age"),
        )
        spouse_age = None
        if mode == "夫妻两人":
            spouse_age = st.number_input(
                ...
                key=_wkey("input_spouse_age"),
            )
        retirement_age = st.number_input(
            "预计退休年龄", min_value=0, max_value=100,
            value=defaults.retirement_age, key=_wkey("input_ret_age"),
        )
        life_expectancy = st.number_input(
            "预计人均寿命", min_value=0, max_value=120,
            value=defaults.life_expectancy, key=_wkey("input_life_exp"),
        )
    ...
    with st.expander("资产与负债"):
        ...
        with col1:
            ...
            deposit = st.number_input(
                ...
                value=float(defaults.finance.deposit) / 10000.0, key=_wkey("fin_deposit"),
            ) * 10000.0
            cash = st.number_input(
                "现金（万元）", min_value=0.0, step=1.0, format="%.2f",
                value=float(defaults.finance.cash) / 10000.0, key=_wkey("fin_cash"),
            ) * 10000.0
            fund_val = st.number_input(
                "基金市值（万元）", min_value=0.0, step=1.0, format="%.2f",
                value=float(defaults.finance.fund_value) / 10000.0, key=_wkey("fin_fund"),
            ) * 10000.0
            stock_val = st.number_input(
                "股票市值（万元）", min_value=0.0, step=1.0, format="%.2f",
                value=float(defaults.finance.stock_value) / 10000.0, key=_wkey("fin_stock"),
            ) * 10000.0
            property_val = st.number_input(
                "房产估值（万元）", min_value=0.0, step=1.0, format="%.2f",
                value=float(defaults.finance.property_value) / 10000.0, key=_wkey("fin_property"),
            ) * 10000.0
            car_val = st.number_input(
                "车辆估值（万元）", min_value=0.0, step=1.0, format="%.2f",
                value=float(defaults.finance.car_value) / 10000.0, key=_wkey("fin_car"),
            ) * 10000.0
            other_assets = st.number_input(
                "其他资产（万元）", min_value=0.0, step=1.0, format="%.2f",
                value=float(defaults.finance.other_assets) / 10000.0, key=_wkey("fin_other_assets"),
            ) * 10000.0
        with col2:
            st.subheader("负债")
            mortgage = st.number_input(
                "房贷余额（万元）", min_value=0.0, step=1.0, format="%.2f",
                value=float(defaults.finance.mortgage_balance) / 10000.0, key=_wkey("fin_mortgage"),
            ) * 10000.0
            car_loan = st.number_input(
                "车贷余额（万元）", min_value=0.0, step=1.0, format="%.2f",
                value=float(defaults.finance.car_loan_balance) / 10000.0, key=_wkey("fin_car_loan"),
            ) * 10000.0
            other_liab = st.number_input(
                "其他负债（万元）", min_value=0.0, step=1.0, format="%.2f",
                value=float(defaults.finance.other_liabilities) / 10000.0, key=_wkey("fin_other_liab"),
            ) * 10000.0

        assets_sum = deposit + cash + fund_val + stock_val + property_val + car_val + other_assets
        liab_sum = mortgage + car_loan + other_liab
        st.caption(f"总资产: {assets_sum / 10000:,.2f} 万 | 总负债: {liab_sum / 10000:,.2f} 万 | 净资产: {(assets_sum - liab_sum) / 10000:,.2f} 万")

    # ======== 3.1.3 持续收入 ========
    with st.expander("持续收入"):
        salary = st.number_input(
            "工资收入（年）", min_value=0.0, step=1000.0, format="%.0f",
            value=float(defaults.income.salary_annual), key=_wkey("inc_salary"),
        )
        rental = st.number_input(
            "房租收入（年）", min_value=0.0, step=1000.0, format="%.0f",
            value=float(defaults.income.rental_annual), key=_wkey("inc_rental"),
        )
        dividend = st.number_input(
            "分红收入（年）", min_value=0.0, step=1000.0, format="%.0f",
            value=float(defaults.income.dividend_annual), key=_wkey("inc_dividend"),
        )
        other_inc = st.number_input(
            "其他收入（年）", min_value=0.0, step=1000.0, format="%.0f",
            value=float(defaults.income.other_annual), key=_wkey("inc_other"),
        )
        income = Income(
    ...
    with st.expander("生活支出"):
        monthly_expense = st.number_input(
            ...
            value=float(defaults.monthly_expense), key=_wkey("exp_monthly"),
        )
        st.markdown("---")
        col_m1, col_m2 = st.columns(2)
        with col_m1:
            mortgage_monthly = st.number_input(
                "月房贷还款", min_value=0.0, step=100.0, format="%.0f",
                value=float(defaults.finance.mortgage_monthly), key=_wkey("exp_mortgage_monthly"),
            )
        with col_m2:
            mortgage_years = st.number_input(
                "房贷剩余年限", min_value=0, max_value=50,
                value=int(defaults.finance.mortgage_years), key=_wkey("exp_mortgage_years"),
            )
        col_c1, col_c2 = st.columns(2)
        with col_c1:
            car_loan_monthly = st.number_input(
                "月车贷还款", min_value=0.0, step=100.0, format="%.0f",
                value=float(defaults.finance.car_loan_monthly), key=_wkey("exp_car_monthly"),
            )
        with col_c2:
            car_loan_years = st.number_input(
                "车贷剩余年限", min_value=0, max_value=20,
                value=int(defaults.finance.car_loan_years), key=_wkey("exp_car_years"),
            )
        loan_annual = (mortgage_monthly + car_loan_monthly) * 12
        st.caption(f"月总支出: {monthly_expense + mortgage_monthly + car_loan_monthly:,.0f} | 年贷款还款: {loan_annual:,.0f}")
    ...
    with st.expander("意外预留"):
        emergency_amount = st.number_input(
            ...
            value=float(defaults.emergency_reserve.amount) / 10000.0, key=_wkey("emerg_amount"),
        ) * 10000.0
        emergency_mode = st.selectbox(
            "分摊方式", ["一次性扣除", "逐年均摊"],
            index=0 if defaults.emergency_reserve.mode == "一次性扣除" else 1,
            key=_wkey("emerg_mode"),
        )
    ...
    with st.expander("经济参数"):
        infl_rate = st.number_input(
            ...
            value=defaults.params.inflation_rate * 100, key=_wkey("param_infl"),
        ) / 100.0
        asset_ret = st.number_input(
            "资产年化收益率（%）", min_value=-5.0, max_value=20.0, step=0.1,
            value=defaults.params.asset_return_rate * 100, key=_wkey("param_asset"),
        ) / 100.0
    ...
    col1, col2 = st.columns(2)
    with col1:
        if st.button("\U0001F504 恢复默认值", help="将所有参数重置为出厂默认值", key=_wkey("btn_reset")):
            ...
            st.rerun()
    with col2:
        if st.button("\U0001F522 重新计算", key=_wkey("btn_recalc")):
            st.session_state.result = None
            st.rerun()
    ...
    with col2:
        ...
        uploaded = st.file_uploader(
            ...
            key=_wkey(f"sidebar_load_{st.session_state.sidebar_upload_key}"),
        )
    ...
    finance = FamilyFinance(
        ...
        mortgage_monthly=mortgage_monthly, mortgage_years=mortgage_years,
        car_loan_monthly=car_loan_monthly, car_loan_years=car_loan_years,
    )
...
def _render_children(existing: list[Child]) -> list[Child]:
    ...
    for i, child in enumerate(existing):
        with st.container():
            ...
            with col2:
                if st.button("\U0001F5D1", key=_wkey(f"del_child_{i}")):
            ...
            age = st.number_input(
                ...
                value=child.current_age, key=_wkey(f"child_{i}_age"),
            )
            ...
            with c1:
                pre_start = st.number_input(
                    ...
                    value=pre_stage.start_age, key=_wkey(f"child_{i}_pre_start"),
                )
            with c2:
                pre_dur = st.number_input(
                    "学前年限", min_value=1, max_value=20,
                    value=pre_stage.duration, key=_wkey(f"child_{i}_pre_dur"),
                )
            with c3:
                pre_cost = st.number_input(
                    "学前年费用", min_value=0.0, step=1000.0, format="%.0f",
                    value=float(pre_stage.annual_cost), key=_wkey(f"child_{i}_pre_cost"),
                )
            ...
            with c1:
                elem_start = st.number_input(
                    ...
                    value=elem_stage.start_age, key=_wkey(f"child_{i}_elem_start"),
                )
            with c2:
                elem_dur = st.number_input(
                    "小学年限", min_value=1, max_value=20,
                    value=elem_stage.duration, key=_wkey(f"child_{i}_elem_dur"),
                )
            with c3:
                elem_cost = st.number_input(
                    "小学年费用", min_value=0.0, step=1000.0, format="%.0f",
                    value=float(elem_stage.annual_cost), key=_wkey(f"child_{i}_elem_cost"),
                )
            ...
            with c1:
                mid_start = st.number_input(
                    ...
                    value=mid_stage.start_age, key=_wkey(f"child_{i}_mid_start"),
                )
            with c2:
                mid_dur = st.number_input(
                    "中学年限", min_value=1, max_value=20,
                    value=mid_stage.duration, key=_wkey(f"child_{i}_mid_dur"),
                )
            with c3:
                mid_cost = st.number_input(
                    "中学年费用", min_value=0.0, step=1000.0, format="%.0f",
                    value=float(mid_stage.annual_cost), key=_wkey(f"child_{i}_mid_cost"),
                )
            ...
            with c1:
                uni_start = st.number_input(
                    ...
                    value=uni_stage.start_age, key=_wkey(f"child_{i}_uni_start"),
                )
            with c2:
                uni_dur = st.number_input(
                    "大学年限", min_value=1, max_value=20,
                    value=uni_stage.duration, key=_wkey(f"child_{i}_uni_dur"),
                )
            with c3:
                uni_cost = st.number_input(
                    "大学年费用", min_value=0.0, step=1000.0, format="%.0f",
                    value=float(uni_stage.annual_cost), key=_wkey(f"child_{i}_uni_cost"),
                )

            sponsorship = st.number_input(
                "毕业后一次性赞助", min_value=0.0, step=1000.0, format="%.0f",
                value=float(child.graduation_sponsorship), key=_wkey(f"child_{i}_sponsor"),
            )
    ...
    if st.button("\u2795 添加小孩", key=_wkey("btn_add_child")):
